...
function EmailComposer({ user, db, getGeminiReply, SYSTEM_INSTRUCTION, onSignOut, onGoHome, setComposer }) {
  const { darkMode } = useTheme();
  // Use a separate collection for email chats
  ...
  useEffect(() => {
    if (!user) return;
    // Use 'email_chats' collection for email composer
    const chatsRef = collection(db, "users", user.uid, "email_chats");
  ...
  useEffect(() => {
    ...
    // Use 'email_chats' collection for messages
    const messagesRef = collection(db, 'users', user.uid, 'email_chats', activeChatId, 'messages');
  ...
  const sendMessage = async () => {
    ...
    if (isNewChat) {
      const newChatRef = await addDoc(collection(db, 'users', user.uid, 'email_chats'), {
    ...
    const messagesRef = collection(db, 'users', user.uid, 'email_chats', currentChatId, 'messages');
    ...
    if (isNewChat) {
      ...
      const chatRef = doc(db, 'users', user.uid, 'email_chats', currentChatId);
  ...
  const handleDeleteChat = async () => {
    ...
    try {
      const messagesRef = collection(db, 'users', user.uid, 'email_chats', chatId, 'messages');
      const messagesSnap = await getDocs(messagesRef);
      for (const docSnap of messagesSnap.docs) {
        await deleteDoc(docSnap.ref);
      }
      const chatRef = doc(db, 'users', user.uid, 'email_chats', chatId);
  ...
  style={isSidebarCollapsed && isMobile ? { width: 0, padding: 0 } : {}}>
        <button className="w-full p-2 mb-4 border border-dashed border-gray-300 rounded-lg bg-transparent text-sm font-medium cursor-pointer transition-all hover:border-green-500 hover:text-green-500 whitespace-nowrap" onClick={handleNewChat}>+ New Chat</button>
        <div className="flex items-center gap-2 mb-5 px-1">
  {searchMode ? (
    <input
      autoFocus
      type="text"
      className="w-full text-xs px-2 py-1 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-200"
    ...
    <button
      className="flex items-center gap-1 text-xs text-gray-500 hover:text-green-500 focus:outline-none"
    ...
    >
      ...
      <span>Search chats</span>
        ...
        <SimpleBar className="flex-grow min-h-0 h-0" style={{ maxHeight: '100%' }}>
          {activeChatId === null && (
            <div className="p-3 rounded-lg cursor-pointer mb-2 text-xs whitespace-nowrap overflow-hidden text-ellipsis bg-green-500 text-white">New Chat</div>
          )}
          {filteredChats.map(chat => (
            <div 
              key={chat.id} 
              className={`relative group p-2 rounded-lg cursor-pointer mb-2 text-xs whitespace-nowrap overflow-hidden text-ellipsis transition-colors flex items-center justify-between ${activeChatId === chat.id ? 'bg-green-500 text-white' : 'hover:bg-gray-200'}`}
            ...
            >
              ...
              <button
                className={`ml-2 p-1 rounded-full hover:bg-green-100 transition-opacity ${
                  isMobile
                    ? 'opacity-100'
                    : sidebarHoverId === chat.id || optionsOpenId === chat.id
                    ? 'opacity-100'
                    : 'opacity-0 group-hover:opacity-100'
                } ${activeChatId === chat.id ? 'text-white' : 'text-green-500'}`}
                ...
                >
                  <div className="absolute left-0 mt-1 w-full bg-white rounded-lg shadow-lg z-20 origin-top">
                    <div className="py-1">
                      ...
                      <div className="flex items-center gap-2 px-4 py-2 text-green-700 font-semibold text-xs tracking-wide rounded-t-lg relative">
                        <span>Communication</span>
                        <span className="flex-1 border-t-2 border-green-500 ml-2"></span>
                      </div>
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'teams' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500`} onClick={() => { if ('email' !== 'teams') { setComposer('teams'); setShowDropdown(false); } }}>Teams Composer</button>
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'email' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500`} onClick={() => { if ('email' !== 'email') { setComposer('email'); setShowDropdown(false); } }} tabIndex={-1}>Email Composer</button>
                      ...
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'grammar' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500`} onClick={() => { if ('email' !== 'grammar') { setComposer('grammar'); setShowDropdown(false); } }}>Writing Editor</button>
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'doc' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500`} onClick={() => { if ('email' !== 'doc') { setComposer('doc'); setShowDropdown(false); } }}>Document Summariser</button>
                      {/* Wellbeing Category */}
                      ...
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'career' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500`} onClick={() => { if ('email' !== 'career') { setComposer('career'); setShowDropdown(false); } }}>Career Advisor</button>
                      <button className={`w-full text-left px-4 py-2 text-sm ${'email' === 'wellbeing' ? 'bg-gray-100 text-gray-900' : 'text-gray-700'} focus:outline-none hover:text-gray-500 rounded-b-lg`} onClick={() => { if ('email' !== 'wellbeing') { setComposer('wellbeing'); setShowDropdown(false); } }}>Wellbeing Assistant</button>
                    </div>
                  </div>
                </Transition>
              </div>
          ...
          <div className="flex-grow p-6 px-8 md:px-24 overflow-y-auto flex flex-col gap-3 ml-2 pb-36 teams-chat-area">
            {messages.length === 0 && !loading && (
              <div className="text-center mt-8">
                ...
                <div className="text-sm text-gray-400 mb-4">Tip: Get suggestions for clear, effective emails.</div>
                <div className="flex flex-wrap justify-center gap-2 mb-2">
                  {[
                    ...
                    <button
                      key={i}
                      className="px-4 py-2 rounded-2xl bg-green-50 text-green-700 text-xs font-medium shadow-sm border border-green-100 hover:bg-green-100 transition-all cursor-pointer teams-sample-button"
                      onClick={() => {
                        setInput(sample);
            ...
            <AnimatePresence initial={false}>
              {[
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex py-3 px-4 rounded-2xl text-sm ${isUser ? 'bg-green-500 text-white self-end' : 'bg-transparent text-black self-start teams-message-bot'}`}
                      style={msg.type === 'bot' || msg.role === 'model' ? { background: 'transparent', border: 'none', boxShadow: 'none' } : {}}
                    >
                      {msg.type === 'bot' || msg.role === 'model' ? (
                        <span
                          className="prose prose-base prose-green max-w-none"
                    ...
                    {(msg.type === 'bot' || msg.role === 'model') && (
                      <div className="flex items-center gap-2 mt-1">
                        <button
                          className="p-1 bg-transparent border-none outline-none hover:bg-green-100 rounded-full transition-colors"
                        ...
                        >
                          <svg className="w-5 h-5 text-green-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                            <rect x="9" y="9" width="13" height="13" rx="2" className="stroke-green-400"/>
                            <rect x="3" y="3" width="13" height="13" rx="2" className="stroke-green-300"/>
                          </svg>
                        </button>
                        {copiedIdx === idx && (
                          <span className="ml-1 text-xs bg-green-50 text-green-700 rounded px-2 py-1 border border-green-100 shadow-sm animate-fade-in">Text copied</span>
                        )}
                      </div>
                    )}
                  </motion.div>
              ...
              <div className="relative w-full max-w-3xl px-4 pb-4 pointer-events-auto">
                ...
                />
                  <div className="flex items-center ml-2 ">
                    <button
                      onClick={sendMessage}
                      className="mb-1 mt-1 mr-1 flex items-center justify-center w-11 h-11 min-w-[44px] min-h-[44px] border-none bg-green-500 text-white rounded-xl cursor-pointer disabled:bg-green-300 transition-colors"
                      style={{ flex: 'none' }}
                      disabled={loading}
                      aria-label="Send"
                    >
