...
function DocumentSummariser({ user, db, getGeminiReply, SYSTEM_INSTRUCTION, onSignOut, onGoHome, setComposer }) {
  ...
  const sendMessage = async () => {
    ...
    if (isNewChat) {
      const newChatRef = await addDoc(collection(db, 'users', user.uid, 'doc_chats'), {
        title: fileName || "New Doc Chat",
        createdAt: serverTimestamp(),
      });
      currentChatId = newChatRef.id;
      setActiveChatId(newChatRef.id);
      // Save file info for new chat
      saveFileInfoToSession(newChatRef.id, fileName, fileText);
    }
    // Always save file info for the current chat
    if (currentChatId) saveFileInfoToSession(currentChatId, fileName, fileText);
    ...
    if (isNewChat) {
      const titleInstruction = `Create a very short, concise chat title (4 words max) for this user prompt: "${fileName}". Do NOT include any formatting, quotes, punctuation, emojis, or special characters like **. Only return the plain title text.`;
      const title = await getGeminiReply([{ role: 'user', parts: [{ text: fileName }] }], titleInstruction);
    ...
    const userMessageForApi = {
      role: 'user',
      parts: [{ text: `Summarize the following document for this request: ${currentInput}\n\nDocument Content:\n${fileText}` }]
    };
    const apiChatHistory = [...messages.map(({ role, parts }) => ({ role, parts })), userMessageForApi];
    const botText = await getGeminiReply(apiChatHistory, SYSTEM_INSTRUCTION || 'You are a document summarizer. Summarize the document with clear headings and bullet points.');
  ...
  style={isSidebarCollapsed && isMobile ? { width: 0, padding: 0 } : {}}>
        <button className="w-full p-2 mb-4 border border-dashed border-gray-300 rounded-lg bg-transparent text-sm font-medium cursor-pointer transition-all hover:border-purple-500 hover:text-purple-500 whitespace-nowrap" onClick={handleNewChat}>+ New Chat</button>
        <div className="flex items-center gap-2 mb-5 px-1">
          {searchMode ? (
            <input
              autoFocus
              type="text"
              className="w-full text-xs px-2 py-1 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-200"
            ...
            <button
              className="flex items-center gap-1 text-xs text-gray-500 hover:text-purple-500 focus:outline-none"
        ...
        <SimpleBar className="flex-grow min-h-0 h-0" style={{ maxHeight: '100%' }}>
          {activeChatId === null && (
            <div className="p-3 rounded-lg cursor-pointer mb-2 text-xs whitespace-nowrap overflow-hidden text-ellipsis bg-purple-500 text-white">New Chat</div>
          )}
          {filteredChats.map(chat => (
            <div
              key={chat.id}
              className={`relative group p-2 rounded-lg cursor-pointer mb-2 text-xs whitespace-nowrap overflow-hidden text-ellipsis transition-colors flex items-center justify-between ${activeChatId === chat.id ? 'bg-purple-500 text-white' : 'hover:bg-gray-200'}`}
            ...
            >
              ...
              <button
                className={`ml-2 p-1 rounded-full hover:bg-purple-100 transition-opacity ${
                  isMobile
                    ? 'opacity-100'
                    : sidebarHoverId === chat.id || optionsOpenId === chat.id
                    ? 'opacity-100'
                    : 'opacity-0 group-hover:opacity-100'
                } ${activeChatId === chat.id ? 'text-white' : 'text-purple-500'}`}
          ...
          <div className="flex-grow p-6 px-8 md:px-24 overflow-y-auto flex flex-col gap-3 ml-2 pb-36 teams-chat-area">
            ...
            <AnimatePresence initial={false}>
              {[
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex py-3 px-4 rounded-2xl text-sm ${isUser ? 'bg-purple-500 text-white self-end' : 'bg-transparent text-black self-start teams-message-bot'}`}
            ...
            {loading && (
              <>
                ...
                <div className="flex items-center self-start gap-2 py-3 px-4">
          ...
          {!(isMobile && !isSidebarCollapsed) && (
            ...
            >
              <div className="relative w-full max-w-3xl px-4 pb-4 pointer-events-auto">
                ...
                <div className="flex items-center justify-between px-3 py-2 mt-1 text-xs text-gray-600 bg-white border border-purple-200 rounded-lg">
