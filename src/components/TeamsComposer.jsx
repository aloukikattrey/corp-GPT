  ...
  const sendMessage = async () => {
    ...
    setOptimisticMessages((prev) => [
      ...prev,
      {
        ...
        optimisticId: `${Date.now()}_${Math.random()}`
      }
    ]);
    if (isNewChat) {
              ...
              >
               ...
               {optionsOpenId === chat.id && (() => {
                 ...
                 return (
                   ...
                   >
                     <button
                       className="w-full z-70 text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 rounded-t-lg"
      ...
      <div className="flex-grow flex flex-col h-[100dvh]">
        <div className="w-full h-full flex flex-col bg-white">
          <div className="p-3 px-6 bg-white border-b border-gray-200 flex justify-between items-center flex-shrink-0 teams-header">
            ...
            {/* Remove user dropdown from header */}
          </div>
          <div className="flex-grow p-6 px-8 md:px-24 overflow-y-auto flex flex-col gap-3 ml-2 teams-chat-area">
            ...
            <AnimatePresence initial={false}>
              {[...messages, ...optimisticMessages].map((msg, idx) => (
                <motion.div
                  ...
                  className={`flex flex-col max-w-[80%] ${msg.type === 'user' ? 'self-end ml-auto items-end' : 'self-start mr-auto items-start'}`}
                  layout
                >
                  <div
                    className={`flex py-3 px-4 rounded-2xl text-sm ${msg.type === 'user' ? 'bg-blue-500 text-white self-end' : 'bg-transparent self-start teams-message-bot'}`}
